...
import { CalendarDays, MapPin, Ticket, Clock, AlertTriangle } from "lucide-react";
...
const MyPasses = () => {
  ...
  const fetchPasses = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/bookings/user/${user?.primaryEmailAddress?.emailAddress}`);
    ...
    <div className="min-h-screen bg-bollywood-dark flex flex-col">
      ...
      <div className="flex-grow py-24 px-4">
        <div className="container mx-auto max-w-4xl">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-8">My Passes</h1>
          ...
          {loading ? (
            ...
            <div className="glass-panel p-8 rounded-xl text-center">
              ...
              <h2 className="text-2xl font-bold text-white mb-4">No Passes Found</h2>
              <p className="text-white/70 mb-6">
                    ...
                    <div>
                      <div className="mb-1">
                        <span className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${
                          pass.status === "CONFIRMED" ? "bg-green-500/20 text-green-400" :
                          pass.status === "USED" ? "bg-gray-500/20 text-gray-400" :
                      ...
                      <h3 className="text-xl font-bold text-white">
                        {pass.ticketType} Pass - {pass.quantity} {pass.quantity > 1 ? "tickets" : "ticket"}
                      </h3>
                      <p className="text-white/70 text-sm">Booking ID: {pass.bookingId}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-bold text-white">₹{pass.totalAmount}</p>
                      <p className="text-white/60 text-xs">
                        Purchased on {new Date(pass.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white/5 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <p className="text-white/70">
                        <span className="text-white/50 text-sm block">Name</span>
                        {pass.name}
                      </p>
                      <p className="text-white/70">
